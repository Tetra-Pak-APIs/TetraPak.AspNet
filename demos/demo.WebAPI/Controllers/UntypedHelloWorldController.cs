        ...
        public async Task<ActionResult> Get(string svc = null)
        {
            ...
            switch (svc.ToLowerInvariant())
            {
                case "tx":
                    ...
                    return await this.RespondAsync(await this.Service("HelloWorld").Endpoint("HelloWorldWithTokenExchange").GetRawAsync());
                
                case "cc": 
                    // note This is an example of how you can use a POC property to fetch the endpoint:
                    return await this.RespondAsync(await this.Service<HelloWorldService>().Endpoints.HelloWorldWithClientCredentials.GetRawAsync());
